...
async function buildGroupReport(supabase, group, todayShift, historyStart) {
  ...
  const recipients = allFFs.filter(f => ['DC', 'Captain'].includes(f.rank) && f.email).map(f => f.email)

  return { rlData: rlRes.data, sickData: sickRes.data, sickMap, todayMap, histData: histRes.data, ffById, refNames, recipients }
}

function buildEmailHtml(group, todayShift, historyDays, { rlData, sickData, sickMap, todayMap, histData, ffById, refNames }) {
  const weekOf = new Date().toLocaleDateString('en-US', { timeZone: 'America/New_York', month: 'long', day: 'numeric', year: 'numeric' })
  const S = `<style>body{font-family:Arial,sans-serif;font-size:13px;color:#000;background:#fff;}
    ...
    .section{margin-bottom:22px;}.sick{font-weight:bold;background:#fff3cd;}.recalled{background:#d4edda;}
    .footer{font-size:11px;color:#666;margin-top:20px;text-align:center;}</style>`

  let html = `<html><head>${S}</head><body>
    <div class="hdr"><h1>HILLSIDE FIRE DEPARTMENT</h1>
    <h2>Weekly Recall Report &mdash; Tour ${group}</h2>
    <p style="font-size:12px;margin:0;">Week of ${weekOf}</p></div>`

  // Section 1
  html += `<div class="section"><h3>SECTION 1 &mdash; CURRENT RECALL ORDER</h3>`
  for (const [label, rows] of [['Captains', rlData.filter(e => e.rank_type === 'Captain')], ['Firefighters', rlData.filter(e => e.rank_type === 'FF')]]) {
    ...
    for (const r of rows) {
      ...
      html += `<tr class="${cls}"><td>${r.list_position}</td><td>${ff.name || ''}</td><td>${ff.rank || r.rank_type}</td><td>${ff.badge_number || '—'}</td><td>${ff.phone || '—'}</td><td>${status}</td></tr>`
    }
    html += `</tbody></table>`
  }
  ...
  if (!histData.length) {
    ...
    for (const r of histData) {
      ...
      html += `<tr><td>${fmtDateShort(r.shift_date)}</td><td>${ff.name || ''} (${ff.rank || ''})</td><td>${recallLabel(r.recall_type)}</td><td>${r.refused_ff_id ? (refNames[r.refused_ff_id] || 'Unknown') : '—'}</td><td>${r.recall_start_time ? fmtTime(r.recall_start_time) : '—'}</td><td>${r.recorded_by || '—'}</td></tr>`
    }
    html += `</tbody></table>`
  }
  html += `</div>`
  ...
  if (!sickData.length) {
    ...
    for (const s of sickData) {
      ...
      html += `<tr><td><b>${ff.name || ''}</b></td><td>${ff.rank || ''}</td><td>${fmtDateShort(s.marked_sick_date?.split('T')[0])}</td><td>${daysOut} day${daysOut === 1 ? '' : 's'}</td><td>${s.notes || '—'}</td></tr>`
    }
    html += `</tbody></table>`
  }
  html += `</div>`

  html += `<p class="footer">Hillside FD system &mdash; ${new Date().toLocaleString('en-US', { timeZone: 'America/New_York' })} ET</p></body></html>`
  return html
}
